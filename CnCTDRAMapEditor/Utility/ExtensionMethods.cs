...
namespace MobiusEditor.Utility {
    public static class ExtensionMethods {
        public static float ToLinear(this float v) => (v < 0.04045f) ? (v * 25.0f / 323.0f) : (float)Math.Pow(((200.0f * v) + 11.0f) / 211.0f, 12.0f / 5.0f);

        public static float ToLinear(this byte v) => (v / 255.0f).ToLinear();

        public static float ToSRGB(this float v) => (v < 0.0031308) ? (v * 323.0f / 25.0f) : ((((float)Math.Pow(v, 5.0f / 12.0f) * 211.0f) - 11.0f) / 200.0f);
        ...
        public static Bitmap Sharpen(this Bitmap bitmap, double strength) {
            ...
            var bias = 1.0 - strength;
            var factor = strength / 16.0;
            ...
            if(sharpenImage != null) {
                var pbits = sharpenImage.LockBits(new Rectangle(0, 0, width, height), ImageLockMode.ReadWrite, PixelFormat.Format24bppRgb);
