...
namespace MobiusEditor.Utility {
    ...
    public static class WWCompression {
        ...
        public static int LCWWorstCase(int datasize) => datasize + (datasize / 63) + 1;

        public static int XORWorstCase(int datasize) => datasize + ((datasize / 63) * 3) + 4;
        ...
        public static byte[] LcwCompress(byte[] input) {
            ...
            var relative = input.Length > ushort.MaxValue;
            ...
            //LCWWorstCase(getend)
            var worstcase = Math.Max(10000, getend * 2);
            ...
            while(getp < getend) {
                ...
                if(getend - getp > 64 && input[getp] == input[getp + 64]) {
                    ...
                    var rlemax = (getend - getp) < ushort.MaxValue ? getend : getp + ushort.MaxValue;
                    var rlep = getp + 1;
                    while(rlep < rlemax && input[rlep] == input[getp])
                        rlep++;

                    var run_length = (ushort)(rlep - getp);
                ...
                var offstart = relative ? getp < ushort.MaxValue ? 0 : getp - ushort.MaxValue : 0;
        ...
        public static byte[] GenerateXorDelta(byte[] source, byte[] @base) {
            ...
            var getbp = 0;
            //Length to process
            var getsendp = Math.Min(source.Length, @base.Length);
            var dest = new byte[XORWorstCase(getsendp)];
            ...
            while(getsp < getsendp) {
                ...
                uint xorcount = 0;
                uint skipcount = 0;
                var lastxor = (byte)(source[getsp] ^ @base[getbp]);
                var testsp = getsp;
                var testbp = getbp;
